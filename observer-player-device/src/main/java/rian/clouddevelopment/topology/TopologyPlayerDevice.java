...
public class TopologyPlayerDevice {
	...
	public Topology getPlayerDeviceJoinFilter() {
		...
		final ObjectMapperSerde<PlayerDeviceSummary> playerDeviceSummarySerder = new ObjectMapperSerde<>(
				PlayerDeviceSummary.class);
		...
		final Materialized<Long, PlayerDeviceSummary, KeyValueStore<Bytes, byte[]>> materializedStore =
			    Materialized.<Long, PlayerDeviceSummary, KeyValueStore<Bytes, byte[]>>as(Global.PLAYERS_DEVICE_SUMMARY)
